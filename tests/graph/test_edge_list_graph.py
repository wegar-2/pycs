...
from pycs.graph.edge_list_graph import EdgeListGraph
from tests.graph.common import populate_empty_graph, make_edges, make_vertices


@fixture
def graph():
    return populate_empty_graph(graph=EdgeListGraph())
...
def test_get_edge(graph):
    assert (
            graph.get_edge(Vertex("A"), Vertex("B")) ==
            Edge(o=Vertex("A"), d=Vertex("B"))
    )
