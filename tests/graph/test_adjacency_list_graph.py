...
@fixture
def graph():
    return populate_empty_graph(graph=AdjacencyListGraph())
...
def test_get_edge(graph):
    # assert graph.get_edge(Vertex("A"), Vertex("B")) == Edge(o=Vertex("A"), d=Vertex("B"))
    assert graph.get_edge(Vertex("A"), Vertex("B")) is None
